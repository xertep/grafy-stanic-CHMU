...
top_frame_inner = tk.Frame(top_frame)
top_frame_inner.pack(anchor="center")

btn_plot = tk.Button(top_frame_inner, text="Zobraz", command=lambda: fetch_and_plot(selected_station.get()))
btn_plot.pack(side="left", padx=5, pady=5)

btn_duk = tk.Button(
    top_frame_inner,
    text="Dukovany",
    bg="#87CEFA",
    activebackground="#6bbbe8",
    width=20,
    height=2,
    command=lambda: fetch_and_plot("Dukovany (B2DUKO01)")
)
btn_duk.pack(side="left", padx=5, pady=5)

btn_brno = tk.Button(
    top_frame_inner,
    text="Brno, Žabovřesky",
    bg="#7fbf7f",
    activebackground="#6aa96a",
    width=20,
    height=2,
    command=lambda: fetch_and_plot("Brno, Žabovřesky (B2BZAB01)")
)
btn_brno.pack(side="left", padx=5, pady=5)
